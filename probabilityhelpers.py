...
#return whether a single var is true
def boolVar(var):
    return random.uniform(0,1) < var

#return the element with highest probability in pdict
def highestProb(pdict):
    pdict_pairs = sorted(pdict.iteritems(), key=lambda (k,v): (v,k))
    return pdict_pairs[-1][0]
...
def probabilityShuffle(pdict):
    order = []
    while len(order) < len(pdict):
        order.append(probDictToChoice(pdict))
        pdict[order[-1]] = 0.0
    return order
